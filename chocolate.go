...
type NChocolate struct {
	...
	// bar tree
	tree *tree.Tree[string, NChocolateBar]

	// navigation
	selectables []string
	selectIdx   int
	selected    NChocolateBar
	focused     bool
}

func (c *NChocolate) AddBar(pid string, bar NChocolateBar) error {
	// TODO: error handling
	...
	for selectable := range c.tree.FindAllBy(
		func(bar NChocolateBar) bool { return bar.IsSelectable() }, false,
	) {
...
func (c NChocolate) IsRoot(bar BarSelector) bool {
	return c.tree.Root().GetData().GetID() == bar.GetID()
}

func (c NChocolate) GetParent(bar BarSelector) BarParent {
...
func (c *NChocolate) GetChildren(bar BarSelector) []BarChild {
	children := []BarChild{}
...
func (c NChocolate) IsSelected(bar BarSelector) bool {
	if c.selected == nil {
		return false
	}
...
func (c NChocolate) IsFocused(bar BarSelector) bool {
...
func (c *NChocolate) Focus() {
	c.focused = true
}

func (c *NChocolate) UnFocus() {
	c.focused = false
}

func (c *NChocolate) handleResize(msg tea.WindowSizeMsg) {
	for bar := range c.tree.FindAllBy(
		func(bar NChocolateBar) bool { return true }, true,
	) {
		bar.GetData().Resize(msg.Width, msg.Height)
	}
}

func (c *NChocolate) handleNavigation(msg tea.Msg) tea.Cmd {
	var cmds []tea.Cmd
...
func (c *NChocolate) Init() tea.Cmd {
	return nil
}

func (c *NChocolate) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmds []tea.Cmd
...
func (c *NChocolate) View() string {
	for bar := range c.tree.FindAllBy(
		func(bar NChocolateBar) bool { return true }, false,
	) {
...
func SetLayout(v LayoutType) chocolateOption {
	return func(c *NChocolate) {
		c.tree.Root().GetData().(BarLayout).SetLayout(v)
	}
}
...
func NewNChocolate(opts ...chocolateOption) (*NChocolate, error) {
	ret := &NChocolate{
		KeyMap: DefaultKeyMap(),
		tree:   tree.NewTree[string, NChocolateBar](),
	}
